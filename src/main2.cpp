...
int main()
{
    ...
    while (true)
    {
        ...
        if (now - pre > 10ms)
        {
            ...
            for (int i = 0; i < motor_amount; i++)
            {
                if (motor_theta[i] - pre_motor_theta[i] > 32767) //繰り上がり、繰り下がり処理
                {
                    motor_theta[i] += 65536; // -32768 + k = 32768
                }
                else if (motor_theta[i] - pre_motor_theta[i] < -32768)
                {
                    motor_theta[i] -= 65536; // 32767 + k = -32769
                }
...
bool readlines2(BufferedSerial &serial, char *buffer, bool is_integar = false, bool is_float = false)
{
    int i = 0;        // 繰り返し変数
    char buff = '0';  // シリアル受信
    string data = ""; // 受信データ保存
